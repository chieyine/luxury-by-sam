...
function normalizeWooProduct(product) {
  return {
    ...
    price: product.price_html
      ? product.price_html.replace(/<[^>]*>?/gm, "")
      : `$${product.price || "0"}`,
    category: product.categories?.[0]?.name || "Collection",
    material: "Material varies",
    image: product.images?.[0]?.src || null,
    sequenceFrames: product.images?.slice(0, 6).map((imageItem) => imageItem.src) || [],
    palette: "linear-gradient(140deg,#ddd5ca,#b8ab96 45%,#85745f)",
    description: product.short_description
      ? product.short_description.replace(/<[^>]*>?/gm, "")
      : "Designed with timeless proportions and premium materials.",
    longDescription:
      product.description?.replace(/<[^>]*>?/gm, "") ||
      "Designed with timeless proportions and premium materials.",
    dimensions: "Dimensions available on request",
    leadTime: "Lead time available on request",
    features: ["Hand-finished construction", "Premium materials", "Craft-led detailing"],
...
export async function getProducts() {
  try {
    ...
    console.error("Falling back to mock products:", err.message);
    return fallbackProducts;
  }
}
